(function(){
    ...
    app.controller('TarefasController', function($scope, $http) {
        ...
        $scope.adicionarTarefa = function () {
            ...
            var requisicao = $http({
                method: "post",
                url: "api/tarefas?api_token="+token,
                data: dadosPost
            }).success(function (data, status) {
                if (data && status == 201) { //status que recebe o status da resposta http recebido de $http
                    $scope.loadData(function () {
                        $scope.texto = '';
                        $scope.autor = '';
                        $scope.status = '';
        ...
        $scope.mudarStatus = function (id, status) {
            ...
            var requisicao = $http({
                method: "put",
                url: "api/tarefas/" + id +"?api_token="+token,
                data: dadosPost
            }).success(function (data, status) {
                if (data.id == id && status == 201) { //status que recebe o status da resposta http recebido de $http
                    $scope.loadData();
                }
                else {
